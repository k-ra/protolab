...
const Hub = (function(){
  ...
  const CLIP = 'protolab.clippings.v1';
  ...
  // Favourites don't go on a shelf — they get pressed onto the Clippings
  // sheet, which is the collection.
  function toClippings(kind, spec){
    const sheet = read(CLIP, null) || {v:1, items: []};
    const isFlower = kind === 'flower';
    sheet.items = sheet.items || [];
    sheet.items.push({
      material: isFlower ? 'plaster' : (spec.substrate || 'wax'),
      form: isFlower ? 'bloom' : 'seal',
      content: 'flower',
      scale: 1, depth: spec.depth || 1, note: '',
      seed: spec.seed || 1,
      die: spec.die || 'monogram',
      text: spec.text || 'K',
      species: spec.species || 'aster'
    });
    write(CLIP, sheet);
    return sheet.items.length;
  }
  function clippingsLink(){ return '../canvas/'; }
  ...
  return {
    ...
    toClippings: toClippings, clippingsLink: clippingsLink
  };
})();
