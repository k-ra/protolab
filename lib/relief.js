...
const Relief = (function(){
  ...
  // Math.pow with a fractional exponent costs ~50-100ns; the shading loop ran
  // two of them per pixel, which was most of its time. This one is a table,
  // and the integer powers below are just multiplications.
  const POW_N = 1024;
  const POW072 = new Float32Array(POW_N + 1);
  for(let i=0;i<=POW_N;i++) POW072[i] = Math.pow(i/POW_N, 0.72);
  ...
  Field.prototype.shade = function(ctx, o){
    ...
    for(let y=0;y<H;y++){
      ...
      for(let x=0;x<W;x++){
        ...
        const u = 1 - nz, u2 = u*u, u5 = u2*u2*u;   // integer powers, not Math.pow

        let sp;
        if(matte && pSpa <= 0.0001){
          // matte is pure diffuse — no specular at all. A whisper of grazing
          // sheen keeps edges alive; the curve lifts midtones so the falloff
          // reads flat rather than modelled.
          let di = (diff*POW_N)|0;
          diff = POW072[di < 0 ? 0 : (di > POW_N ? POW_N : di)];
          sp = u5*pSheen*atten;
        } else {
        ...
        const sc   = pScat > 0 ? u2*Math.sqrt(u)*pScat*(0.35 + 0.65*atten) : 0;
