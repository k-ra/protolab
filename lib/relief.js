    ...
    for(let y=0;y<H;y++){
      ...
      for(let x=0;x<W;x++){
        ...
        if(matte){
          ...
          sp = Math.pow(1 - nz, 5.0)*sheenAmt*atten;
        } else {
          let hx = lx, hy = ly, hz = lz + 1;
          const hn = 1/Math.sqrt(hx*hx + hy*hy + hz*hz);
          const ndh = (nx*hx + ny*hy + nz*hz)*hn;
          sp = ndh > 0 ? Math.pow(ndh, shin)*spa*atten : 0;
          sp = sp/(1 + sp*0.8);
        }

        const ao = aoLo + aoHi*Math.min(1, hgt[i]*invH*1.3);
        ...
        if(useEnv){
          ...
          t = t < 0 ? 0 : (t > 1 ? 1 : t);
          t = t*t*(3 - 2*t);
          envMul = envLo + (envHi - envLo)*t;
        }

        let b0 = c0[0], b1 = c0[1], b2 = c0[2];
        if(cmap){ const m = i*3; b0 = cmap[m]; b1 = cmap[m+1]; b2 = cmap[m+2]; }

        const base = amb * ao * envMul * exp;
        const key  = (1-amb) * diff * ao * envMul * exp;
        const sc   = scat > 0 ? Math.pow(1 - nz, 2.4)*scat*(0.35 + 0.65*atten) : 0;
